...
async function getDownloadList(){
    try{
        ...
        const res = await axios.get('http://3.108.252.114:3000/expense/downloadList',{headers:{'Authorization':token}}); 
    }
    catch(err){
        console.log(err);
    }
...
async function createExpenseDownLoadList(){
    try{
        ...
        if(isPremium==null && isPremium===false)
            return;
        const res = await axios.get('http://3.108.252.114:3000/expense/downloadHistory',{headers:{'Authorization':token}});
...
async function createExpenseItemList(currentPage){
    ...
    try{
        ...
        const res = await axios.get('http://3.108.252.114:3000/expense',{params:{currentPage:+currentPage,expensesPerPage: +expensePerPage},headers:{'Authorization':token}});
...
async function deleteExpenseListItem(obj){
    ...
    const token = localStorage.getItem('userToken');
    try{
        await axios.delete(`http://3.108.252.114:3000/expense/${id}`,{headers:{'Authorization':token}})
...
expenseForm.addEventListener('submit',async (e)=>{
    ...
    try{
        const token = localStorage.getItem('userToken');
        const res = await axios.post('http://3.108.252.114:3000/expense',{
